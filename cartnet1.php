...
foreach($_POST['quantity'] as $key => $val) {
    if($val==0) {
        unset($_SESSION['cartnet1'][$key]);
    }else{
        $_SESSION['cartnet1'][$key]['quantity']=$val;
    }
}
  
}
?>
<form method="post" action="cartgame2.php?page=cartnet1">
...
<tr class="even">
        ...
        <?php
			...
			$sql="SELECT * FROM networking WHERE id_product IN (";
                    ...
                    while($row=mysqli_fetch_array($query)){
                        $subtotal=$_SESSION['cartnet1'][$row['id_product']]['quantity']*$row['price'];
                        $totalprice+=$subtotal;
						$grandtotal=$totalprice+$totalprice*$tax;
                    ?>
                        <tr>
                            <td><?php echo $row['product_name'] ?></td>
               <td><input type="text" name="quantity[<?php echo $row['id_product'] ?>]" size="5" value="                            <?php echo $_SESSION['cartnet1'][$row['id_product']]['quantity'] ?>" /></td>
                            <td>Ksh.<?php echo $row['price'] ?></td>
                            <td>Ksh.<?php echo $_SESSION['cartnet1'][$row['id_product']]['quantity']*$row['price'] ?></td>
                        </tr>
                    <?php
    ...
    <table>
    <td width="350"><a href="cartnet2.php?page=listing_3"><img src="images/pic_14.png" alt="Continue shopping""</a></h2></td>
