          ...
          <li> <a href="listing_2.html">Computers</a>
            <div class="s_submenu">
              ...
              <ul class="s_list_1 clearfix">
                ...
                <li><a href="listing_2.html">Tablets</a></li>
                <li><a href="listing_2.html">Accessories</a></li>
                <li><a href="listing_2.html">Software</a></li>
...
foreach($_POST['quantity'] as $key => $val) {
    if($val==0) {
        unset($_SESSION['cartreal1'][$key]);
    }else{
        $_SESSION['cartreal1'][$key]['quantity']=$val;
    }
}
  
}
?>
<form method="post" action="cartreal2.php?page=cartreal1">
...
<tr class="even">
        ...
        <?php
			...
			$sql="SELECT * FROM realestate WHERE id_product IN (";
                      
                    foreach($_SESSION['cartreal1'] as $id => $value) {
                        $sql.=$id.",";
                    }
                    ...
                    while($row=mysqli_fetch_array($query)){
                        $subtotal=$_SESSION['cartreal1'][$row['id_product']]['quantity']*$row['price'];
                        $totalprice+=$subtotal;
						$grandtotal=$totalprice+$totalprice*$tax;
                    ?>
                        <tr>
                            <td><?php echo $row['product_name'] ?></td>
               <td><input type="text" name="quantity[<?php echo $row['id_product'] ?>]" size="5" value="                            <?php echo $_SESSION['cartreal1'][$row['id_product']]['quantity'] ?>" /></td>
                            <td>Ksh.<?php echo $row['price'] ?></td>
                            <td>Ksh.<?php echo $_SESSION['cartreal1'][$row['id_product']]['quantity']*$row['price'] ?></td>
                        </tr>
                    <?php
        ...
        ?>
                    <tr>
                        <td colspan="3"><strong>Total Price:</strong>Ksh. <?php echo $totalprice ?></td>
                        <td><strong>Grand Price with VAT:</strong>Ksh. <?php echo $grandtotal ?></td>
                    </tr>
          
    </table>
    <center><strong><p>To remove an item, set it's quantity to 0 and then update cart </p></strong></center>
  
    <table>
    <td width="350"><a href="cartreal2.php?page=listing_6"><img src="images/pic_14.png" alt="Continue shopping""</a></h2></td>
    <td width="350"><button type="submit" name="submit">UPDATE CART</button></form></td>
    <td><a href="checkout.html"><img src="images/pic13.gif" alt="Buy now""</a></h2></td></center>
