...
foreach($_POST['quantity'] as $key => $val) {
    if($val==0) {
        unset($_SESSION['cartcomp'][$key]);
    }else{
        $_SESSION['cartcomp'][$key]['quantity']=$val;
    }
}
  
}
?>
<form method="post" action="cartcom.php?page=cartcomp">
...
<tr class="even">
        ...
        <?php
			...
			$sql="SELECT * FROM computers WHERE id_product IN (";
                    ...
                    while($row=mysqli_fetch_array($query)){
                        $subtotal=$_SESSION['cartcomp'][$row['id_product']]['quantity']*$row['price'];
                        $totalprice+=$subtotal;
						$grandtotal=$totalprice+$totalprice*$tax;
                    ?>
                        <tr>
                            <td><?php echo $row['product_name'] ?></td>
               <td><input type="text" name="quantity[<?php echo $row['id_product'] ?>]" size="5" value="                            <?php echo $_SESSION['cartcomp'][$row['id_product']]['quantity'] ?>" /></td>
                            <td>Ksh.<?php echo $row['price'] ?></td>
                            <td>Ksh.<?php echo $_SESSION['cartcomp'][$row['id_product']]['quantity']*$row['price'] ?></td>
                        </tr>
                    <?php
    ...
    <table>
    <td width="350"><a href="cartcom.php?page=listing_2"><img src="images/pic_14.png" alt="Continue shopping""</a></h2></td>
