...
foreach($_POST['quantity'] as $key => $val) {
    if($val==0) {
        unset($_SESSION['cartsoft1'][$key]);
    }else{
        $_SESSION['cartsoft1'][$key]['quantity']=$val;
    }
}
  
}
?>
<form method="post" action="cartsoft2.php?page=cartsoft1">
...
<tr class="even">
        ...
        <?php
			...
			$sql="SELECT * FROM software WHERE id_product IN (";
                      
                    foreach($_SESSION['cartsoft1'] as $id => $value) {
                        $sql.=$id.",";
                    }
                    ...
                    while($row=mysqli_fetch_array($query)){
                        $subtotal=$_SESSION['cartsoft1'][$row['id_product']]['quantity']*$row['price'];
                        $totalprice+=$subtotal;
						$grandtotal=$totalprice+$totalprice*$tax;
                    ?>
                        <tr>
                            <td><?php echo $row['product_name'] ?></td>
               <td><input type="text" name="quantity[<?php echo $row['id_product'] ?>]" size="5" value="                            <?php echo $_SESSION['cartsoft1'][$row['id_product']]['quantity'] ?>" /></td>
                            <td>Ksh.<?php echo $row['price'] ?></td>
                            <td>Ksh.<?php echo $_SESSION['cartsoft1'][$row['id_product']]['quantity']*$row['price'] ?></td>
                        </tr>
                    <?php
    ...
    <table>
    <td width="350"><a href="cartsoft2.php?page=listing_5"><img src="images/pic_14.png" alt="Continue shopping""</a></h2></td>
